...
public class PlayerRestrictions extends AbstractRestrictions {
	...
	public boolean canAffectBySkill(Player player, VisibleObject target) {
		...
		if ((skill == null) || (target instanceof Player && ((Player) target).isProtectionActive())) {
			return false;
		}
	...
	public boolean canUseSkill(Player player, Skill skill) {
		...
		if (player.isCasting() || ((!player.canAttack()) && (template.getSkillId() != 1968))) {
			return false;
		}
	...
	public boolean canAttack(Player player, VisibleObject target) {
		...
		if ((target == null) || player.isUsingFlyTeleport()
				|| (target instanceof Player && ((Player) target).isUsingFlyTeleport())) {
			return false;
		}
