...
public class StatFunctions {
	...
	public static int calculatePvEApGained(Player player, Creature target) {
		...
		return lvlDiff ? 1
				: (int) Math.floor(15 * apPercentage * player.getRates().getApNpcRate() * ApNpcRating(npcRating) / 100);
	}
	...
	public static int adjustPvpDpGained(int points, int defeatedLvl, int killerLvl) {
		...
		if (difference >= 10) {
			...
			pointsGained -= (int) (pointsGained * difference * 0.1);
		} else if (difference <= -10) {
			pointsGained *= (int) 1.1;
		} else if (difference > -10 && difference < 0) {
			pointsGained += (int) (pointsGained * Math.abs(difference) * 0.01);
		}
	 ...
	 * @return Damage made to target (-hp value)
	 */
	public static int calculatePhysicalAttackDamage(Creature attacker, Creature target, Func func, float skillDamages,
			int bonus, boolean isMainHand, int pvpDamage) {
		Stat2 pAttack;
		if (isMainHand) {
		...
		if (attacker instanceof Player) {
			...
			if (weapon != null) {
				...
				resultDamage = pAttack.getBonus()
						+ getMovementModifier(attacker, StatEnum.PHYSICAL_ATTACK, pAttack.getBase());
	...
	public static int calculateMagicalSkillDamage(Creature speller, Creature target, int baseDamages, int bonus,
			SkillElement element, boolean useKnowledge, boolean noReduce, int pvpDamage) {
	...
	public static boolean calculatePhysicalDodgeRate(Creature attacker, Creature attacked, int accMod) {
		...
		if (attacker.getObserveController().checkAttackerStatus(AttackStatus.DODGE)
				|| attacked.getObserveController().checkAttackStatus(AttackStatus.DODGE)) {
			return true;
		}
	...
	public static boolean calculatePhysicalCriticalRate(Creature attacker, Creature attacked, boolean isMainHand,
		...
		// add critical Prob
		critical *= (int) (criticalProb / 100f);
