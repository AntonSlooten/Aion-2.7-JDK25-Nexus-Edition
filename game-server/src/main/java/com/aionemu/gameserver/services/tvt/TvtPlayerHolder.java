...
public class TvtPlayerHolder {
	...
	private FastList<PlayerGroup> groups = new FastList<>();
	private FastList<PlayerGroup> asmosGroupsReady = new FastList<>();
	private FastList<PlayerGroup> elyosGroupsReady = new FastList<>();
	private int asmo;
	private int elys;
	private int asmoReg;
	private int elysReg;
	...
	public void makeGroup() {
		...
		FastList<Player> asmodians = new FastList<>();
		FastList<Player> elyos = new FastList<>();
		if (players == null) {
	...
	private void makeNewGroupAndAddMemberForElyos(FastList<Player> elyos) {
		if (elyos.size() > 1) {
			...
			for (Player player : elyos) {
				if ((player.getPlayerGroup2() == groupForElyos) || groupForElyos.isFull()) {
					continue;
				}
	...
	private PlayerGroup makeOneGroup(Player inviter, Player invited) {
		inviter.setPlayerGroup2(new PlayerGroup(member)); // inviter.setPlayerGroup(new
															// PlayerGroup(IDFactory.getInstance().nextId(), inviter));
															// original
	...
	public void info(Player player, boolean admin) {
		...
		TvtPlayerHolder.sentUnRegMess(player,
				"----------------------------------------\n|       [Ascension]Tvt Info:       |\n----------------------------------------\nTime remaining:"
						+ Math.round(reg.getRemainingTime() / 60) + 1 + "m.\n");
		String raceinfo = "";
		switch (player.getRace()) {
		case ASMODIANS:
			raceinfo = "Asmodians ";
		...
		for (Player p : players) {
			if (p.getRace() == player.getRace()) {
				racereg++;
				sentUnRegMess(player, p.getName() + " (" + p.getCommonData().getLevel() + ") ["
						+ p.getCommonData().getPlayerClass() + "]\n");
			}
		}
		if (admin) {
			TvtPlayerHolder.sentUnRegMess(player,
					"Registered " + otherRace + "----------------------------------------\n");

			for (Player p : players) {
				if (p.getRace() != player.getRace()) {
					TvtPlayerHolder.sentUnRegMess(player, p.getName() + " (" + p.getCommonData().getLevel() + ") ["
							+ p.getCommonData().getPlayerClass() + "]\n");
				}
			}
		}
	...
	public void sendSorryMessage() {
		for (Player player : players) {
			PacketSendUtility.sendWhiteMessageOnCenter(player,
					"[Ascension]Tvt: Sorry,For Tvt not Enough Player,Aborting...");
		}
	}
