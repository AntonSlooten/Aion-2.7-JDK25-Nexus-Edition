	...
	public Player readInfo(String name, int targetAccount, String accountName, List<Integer> rsList, Logger textLog) {
		...
		for (int a = 0; a < cnt; a++) { // inventory
			...
			if (PlayerTransferConfig.ALLOW_INV) {
				...
				Item item = new Item(newId, itemId, itemCnt, itemColor, itemCreator, itemExpireTime, itemActivationCnt,
						itemEquipped, itemSoulBound, equipSlot, location, enchant, skinId, fusionId, optSocket,
						optFusion, charge);
		...
		for (int a = 0; a < cnt; a++) { // quests
			...
			if (PlayerTransferConfig.ALLOW_QUESTS) {
				player.getQuestStateList().addQuest(questId, new QuestState(questId, QuestStatus.valueOf(readS()),
						readD(), readD(), readD(), null, readD(), null)); // TODO null timestamp
			} else {
				readS();
				readB(12);
			}
