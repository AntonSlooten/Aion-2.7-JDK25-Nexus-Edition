...
public class DialogService {
	...
	public static void onDialogSelect(int dialogId, final Player player, Npc npc, int questId,
				...
				if (qs != null) {
					if (qs.getStatus() == QuestStatus.START || qs.getStatus() == QuestStatus.REWARD) {
						if (AdminConfig.QUEST_DIALOG_LOG) {
							log.info("Error in the quest " + questId + ". No response from " + npc.getNpcId()
									+ " on the step " + qs.getQuestVarById(0));
						}
		...
		case 54: { // coin reward (2.5)
			...
			// PacketSendUtility.sendPacket(player, new SM_MESSAGE(0, null, "This feature is
			// not available yet",
