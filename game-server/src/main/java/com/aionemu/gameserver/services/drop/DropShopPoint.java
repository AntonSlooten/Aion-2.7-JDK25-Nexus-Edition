...
public class DropShopPoint {
	...
	public void callNpcShopPointRewardForPlayer() {
		...
		Player player = null;
		PlayerGroup pg = null;
		Collection<Player> listPlayer = new ArrayList<>();
		...
		/*
		 * if (player.getLevel() - npc.getLevel() >= 10) { // Your level is too high for
		 * winning Shop Point sendCommandMessage(player, message); return; }
		 */
		...
		if (pg == null) {
			...
			log.info("Player " + player.getName() + " has win " + amount + " shop point from NPC " + npc.getName()
					+ " (ID " + npc.getNpcId() + ")");
			/*
			 * // An error has occured when adding your Shop Point. Please contact
			 * administrator String message =
			 * CommandsString.DSP_ERROR_ADD_ERROR.toString(player);
