...
public class DecomposeAction extends AbstractItemAction {
	...
	static {
		...
		manastones.put(30,
				new int[] { 167000290, 167000291, 167000292, 167000293, 167000294, 167000295, 167000296, 167000297,
						167000299, 167000450, 167000451, 167000452, 167000453, 167000454, 167000455, 167000456,
						167000457, 167000459 });
		manastones.put(40,
				new int[] { 167000322, 167000323, 167000324, 167000325, 167000327, 167000328, 167000329, 167000331,
						167000482, 167000483, 167000484, 167000485, 167000487, 167000488, 167000489, 167000491,
						167000539, 167000540 });
		manastones.put(50,
				new int[] { 167000354, 167000355, 167000356, 167000357, 167000358, 167000359, 167000360, 167000361,
						167000363, 167000514, 167000515, 167000516, 167000517, 167000518, 167000519, 167000520,
						167000521, 167000522, 167000541, 167000542 });
		manastones.put(60,
				new int[] { 167000543, 167000544, 167000545, 167000546, 167000547, 167000548, 167000549, 167000550,
						167000551, 167000552, 167000553, 167000554, 167000555, 167000556, 167000557, 167000558,
						167000560, 167000561 });
	}
	...
	public void act(final Player player, final Item parentItem, final Item targetItem) {
		...
		PacketSendUtility.broadcastPacketAndReceive(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(),
				parentItem.getObjectId(), parentItem.getItemId(), USAGE_DELAY, 0, 0));
		player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			boolean postValidate(Player player, Item parentItem) {
				if (!canAct(player, parentItem, targetItem) || player.getLifeStats().isAlreadyDead()
						|| !player.isSpawned()) {
					return false;
				}
	...
	private Collection<ExtractedItemsCollection> filterItemsByLevel(Player player,
		...
		for (ExtractedItemsCollection collection : itemsCollections) {
			if ((collection.getMinLevel() > playerLevel)
					|| (collection.getMaxLevel() > 0 && collection.getMaxLevel() < playerLevel)) {
				continue;
			}
