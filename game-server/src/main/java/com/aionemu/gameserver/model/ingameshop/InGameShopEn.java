...
public class InGameShopEn {
	...
	public FastList<Integer> getTopSales(int category) {
		...
		for (IGItem item : this.items.values()) {
			if ((item.getSalesRanking() == 0) || (category != 2 && item.getCategory() != category)) {
				continue;
			}
		...
		if (CustomFun.CUSTOM_RANK_ENABLED && item.getList() != 0) {
			...
			if (result) {
				...
				PacketSendUtility.sendMessage(player,
						"Not enough coin of rank : " + CustomPlayerRank.getCoinName(item.getList(), player));
			}
			return;
		}
		lastRequestId++;
					...
					if (result == 1) {
						...
						if (request.gift) {
							SystemMailService.getInstance().sendMail(player.getName(), request.receiver, "In Game Shop",
									request.message, item.getItemId(), item.getItemCount(), 0, false);
							PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_MSG_INGAMESHOP_GIFT_SUCCESS);
							if (LoggingConfig.LOG_INGAMESHOP) {
								log.info("[INGAMESHOP] > Account name: " + player.getAcountName() + ", PlayerName: "
										+ player.getName() + " BUY ITEM: " + item.getItemId() + " COUNT: "
										+ item.getItemCount() + " FOR PlayerName: " + request.receiver);
							}
						} else {
							ItemService.addItem(player, item.getItemId(), item.getItemCount());
							if (LoggingConfig.LOG_INGAMESHOP) {
