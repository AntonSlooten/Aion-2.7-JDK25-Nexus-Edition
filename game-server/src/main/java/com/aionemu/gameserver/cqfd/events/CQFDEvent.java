...
public class CQFDEvent {
	...
	public static final ArrayList<CQFDEvent> _events = new ArrayList<>();
	...
	public HashMap<Integer, EventRewards> _rewards = new HashMap<>();
	public HashMap<Integer, CQFDEventTeam> _teams = new HashMap<>();

	public ArrayList<CQFDEventPlayer> _spawns = new ArrayList<>();

	public int _registerDelay = 0;

	private HashMap<CQFDCOUNTERTYPE, ArrayList<CQFDTeamCounter>> _counters = new HashMap<>();
	...
	private HashMap<Integer, ArrayList<CQFDEventTask>> _taskList = new HashMap<>();
	private ArrayList<ScheduledFuture<?>> _currentTasks = new ArrayList<>();
	...
	public void initVars() {
		for (CQFDCOUNTERTYPE type : CQFDCOUNTERTYPE.values()) {
			_counters.put(type, new ArrayList<>());
		}
	}
	...
	/*
	 * public int getReflectionId(){ return _reflection.getId(); }
	 */
	...
	public void registerStep(Collection<CQFDEventTeam> teams, CQFDListenerType lisener, CQFDORDER order,
			CQFDCOUNTERTYPE type, int value, int stepId) {
	...
	public void registerStep(Collection<CQFDEventTeam> teams, CQFDListenerType lisener,
			Class<? extends CQFDEventTask> task, int eatch, CQFDORDER order, CQFDCOUNTERTYPE type, int value,
			int stepId) {
		for (CQFDEventTeam team : teams) {
	...
	public void startRegistrationTask(int registrationTime) {
		...
		_currentScheduledTask = ThreadPoolManager.getInstance()
				.schedule(new CQFDEventTaskRegister(this, registrationTime), 1000);
	}
	...
	public void startEventTimeOut() {
		...
		if (_maxEventTime != -1) {
			_timeOutTask = ThreadPoolManager.getInstance().schedule(new CQFDEventTimeOutTask(this),
					_maxEventTime * 1000L);
		}
	}
	...
	public void sendStepUpdateToTasks() {
		...
		for (CQFDEventTask c : tasks) {
			if (c.isFixedRate()) {
				_currentTasks
						.add(ThreadPoolManager.getInstance().scheduleAtFixedRate(c, c.getInitDelay(), c.getDelay()));
			} else {
	...
	/**
	 * Create Team if not exist and set admin pos for TeamSpawn else if exist update
	 * TeamSpawn with admin pos
	 *
	 * @param admin
	 * @param id
	 * @return
	...
	/**
	 * teleportBack all player in this team and destroy team (teamspawn)
	 *
	 * @param id
	 */
	...
	/**
	 * teleportBack all player in all team and destroy all teams (teamspawn)
	 */
	...
	/**
	 * add Player to teamId if already in other team set his team
	 *
	 * @param player
	 * @param teamId
	 * @param admin
