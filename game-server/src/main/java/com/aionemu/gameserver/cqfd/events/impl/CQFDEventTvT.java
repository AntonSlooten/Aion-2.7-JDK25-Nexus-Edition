...
public class CQFDEventTvT extends CQFDEvent {
	...
	private final static ArrayList<CQFDEventPlayer> SPAWNS = new ArrayList<>(Arrays.asList(
	...
	public static int[][][] REWARDS = { { // REWARD_LOSE
			{ 57, 1 }, { 57, 1 } },
			{ // REWARD_WIN
					{ 57, 2 }, { 57, 2 } } };
	...
	public void init() {
		...
		// start kill counter
		registerStep(getTeams().values(), CQFDListenerType.PLAYER_KILL_PLAYER, CQFDORDER.INC, CQFDCOUNTERTYPE.INFINITE,
				1, 1);
		...
		// CQFDORDER.INC, CQFDCOUNTERTYPE.TEMPORARY, 10, 2);
	...
	public void counterOnTimeOut(final HashMap<CQFDCOUNTERTYPE, ArrayList<CQFDTeamCounter>> finalCounters) {
		...
		// reward player here
		long bestScore = 0;
		final ArrayList<CQFDEventTeam> winners = new ArrayList<>(2);
		final ArrayList<CQFDEventTeam> losers = new ArrayList<>();
