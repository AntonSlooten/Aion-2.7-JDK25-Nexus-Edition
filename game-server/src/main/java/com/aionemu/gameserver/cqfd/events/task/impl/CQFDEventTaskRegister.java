...
public class CQFDEventTaskRegister extends CQFDEventTask {
	...
	private final static ArrayList<Player> registered = new ArrayList<>();
	...
	public void run() {
		...
		_currentScheduledTask = ThreadPoolManager.getInstance().schedule(new CQFDEventTaskRegisterEnd(getEvent()),
				_registrationTime * 1000L);
	}
	 ...
	 */
	/*
	 * public static void question() { _log.info("question"); for(Player player :
	 * GameObjectsStorage.getAllPlayersForIterate()) if(player != null &&
	 * !player.isDead() && player.getReflection().isDefault() &&
	 * !player.isInOlympiadMode() && !player.isInObserverMode() &&
	 * player.getVar("jailed") == null) player.scriptRequest(new
	 * CustomMessage("scripts.cqfd.events.invade.AskPlayer", player).toString(),
	 * "cqfd.events.task.impl.CQFDEventTaskRegister:addPlayer", new Object[0]); }
	 */
	...
	public static void addPlayer(Player player) {
		...
		// show(new CustomMessage("scripts.cqfd.events.invade.Registered", player),
		// player);
	}
	...
	private static boolean checkDualBox(Player player) {
		 ...
		 * if(!CQFDConfig.EVENT_InvadeAllowMultiReg) {
		 * if("IP".equalsIgnoreCase(CQFDConfig.EVENT_InvadeCheckWindowMethod)) {
		 * if(boxes.containsValue(player.getIP())) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.CancelledBox", player), player);
		 * return false; } }
		 * 
		 * else if("HWid".equalsIgnoreCase(CQFDConfig.EVENT_InvadeCheckWindowMethod)) {
		 * if(boxes.containsValue(player.getNetConnection().getHWID())) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.CancelledBox", player), player);
		 * return false; } } }
		 */
		return true;
	...
	public static boolean checkPlayer(Player player, boolean first) {
		/*
		 * if(first && !_isRegistrationActive) { show(new
		 * CustomMessage("scripts.events.Late", player), player); return false; }
		 * 
		 * if(first && (players_list.contains(player))) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.Cancelled", player), player);
		 * if(players_list.contains(player.getStoredId()))
		 * players_list.remove(player.getStoredId());
		 * if(boxes.containsKey(player.getStoredId()))
		 * boxes.remove(player.getStoredId()); return false; }
		 * 
		 * if(first && player.isDead()) return false;
		 * 
		 * if(first && (players_list.contains(player) ||
		 * players_list.contains(player.getStoredId()))) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.Cancelled", player), player);
		 * return false; }
		 * 
		 * if(player.isMounted()) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.Cancelled", player), player);
		 * return false; }
		 * 
		 * if(player.isInDuel()) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.CancelledDuel", player), player);
		 * return false; }
		 * 
		 * if(player.getTeam() != TeamType.NONE) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.CancelledOtherEvent", player),
		 * player); return false; }
		 * 
		 * if(player.isInOlympiadMode() || first && Olympiad.isRegistered(player)) {
		 * show(new CustomMessage("scripts.cqfd.events.invade.CancelledOlympiad",
		 * player), player); return false; }
		 * 
		 * if(player.isInParty() && player.getParty().isInDimensionalRift()) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.CancelledOtherEvent", player),
		 * player); return false; }
		 * 
		 * if(player.isTeleporting()) { show(new
		 * CustomMessage("scripts.cqfd.events.invade.CancelledTeleport", player),
		 * player); return false; }
		 */
		return true;
	}
