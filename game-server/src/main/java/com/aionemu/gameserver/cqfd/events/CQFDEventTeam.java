...
public class CQFDEventTeam {
	 ...
	 * private Location _teamSpawn; private final CQFDEvent _event;
	 * 
	 * public CQFDEventTeam(CQFDEvent event, Location loc){ _event = event;
	 * _teamSpawn = loc; }
	 * 
	 * public void addMember(Player player){ if(!_registrationList.contains(player))
	 * _registrationList.add(player); }
	 * 
	 * public CQFDEvent getEvent(){ return _event; }
	 * 
	 * public boolean contain(Player player){ return _members.containsKey(player); }
	 * public void removeMember(Player player){ System.out.println("[removeMember]"+
	 * player.getName()); Location loc = _members.get(player); if(loc == null){
	 * System.out.println("[removeMember] loc null"); return; }
	 * if(_event.getEventStat() != CQFDEventStat.IDLE){
	 * System.out.println("[removeMember] teleport"); player.teleToLocation(loc,
	 ...
	 * 
	 * 
	 * public void start(){ for(Player player: _registrationList) if(player !=
	 * null){ _members.put(player , player.getLoc());
	 * player.teleToLocation(_teamSpawn, _event.getReflectionId()); } }
	 * 
	 * public void stop(){ for(Player player : _members.keySet()) if(player !=
	 * null){ System.out.println("[Event] remove : "+ player.getName());
	 * removeMember(player); } }
	 * 
	 * ////////////////////////////////////////////
	 * ////////////////////////////////////////////
	 * 
	 * public void giveReward(int[][] rewards){ for(int i = 0 ; i < rewards.length;
	 * i++) for(Player player : _members.keySet()) if(player != null){
	 * System.out.println("[Event] giveReward at "+ player.getName()+ " count "+
	 * rewards[i][1]); player.getInventory().addItem(rewards[i][0], rewards[i][1]);
	 * }
	 * 
	 * }
	 * 
	 ...
	 */
	public final ArrayList<CQFDEventPlayer> _players = new ArrayList<>();
	...
	public void stop(Boolean balance) {
		...
		/*
		 * for(CQFDEventPlayer ev : _players){ eventTools.teleportBackPlayer(this, ev);
		 * if(balance) equip(ev); }
		 */
	}
