...
public class eventTools {
	...
	public static void announceRegisterEnd(CQFDEvent event) {
		for (Player player : World.getInstance().getAllPlayers()) {
			PacketSendUtility.sendBrightYellowMessageOnCenter(player,
					"[Event] " + event.getEventName() + " register ended");
		}
	}

	/*
	 * public static void teleportPlayer(CQFDEventTeam team , CQFDEventPlayer ep,
	 * CQFDEventPlayer loc){ if(ep._player == null){ team._players.remove(ep);
	 * return; } if(ep._player.getLifeStats().isAlreadyDead()) resu(team,
	 * ep._player);
	 * 
	 * 
	 * //ep._player.setEventTeamId(_team._teamId); teleportPlayer(team, ep, loc); }
	 */

	public static boolean deequip(CQFDEventTeam team, CQFDEventPlayer ep) {
		// final ItemInstance[] equipments =
		// ep._player.getInventory().getPaperdollItems();
	...
	/*
	 * public static void createLeague(CQFDEventTeam team){ try{
	 * 
	 * //if(_players.size() <= 1 && _players.size() > 192) //LIMITE? // return;
	 * 
	 * 
	 * int playerInGroup = 0; Player GroupLeader = null; Party Group = null;
	 * 
	 * Player ChannelCoLeader = null; CommandChannel ChannelCo = null;
	 * 
	 * for(CQFDEventPlayer ep : team._players){ if(ep._player == null) return;
	 * 
	 * playerInGroup++;
	 * 
	 * expelFromGroup(team, ep._player);
	 * 
	 * if(playerInGroup == 9) playerInGroup = 1;
	 * 
	 * //store leader if(playerInGroup == 1){ //if league not create and one
	 * alliance already create if(ChannelCoLeader == null && GroupLeader != null)
	 * ChannelCoLeader = GroupLeader;//save league leader GroupLeader = ep._player;
	 * continue; }
	 * 
	 * else if(playerInGroup == 2){//create party GroupLeader.setParty(Group = new
	 * Party(GroupLeader, 0)); //if ChannelCoLeader selected and ChannelCo not
	 * create if(ChannelCo != null) Group.setCommandChannel(ChannelCo); else
	 * if(ChannelCoLeader != null) Group.setCommandChannel(ChannelCo = new
	 * CommandChannel(GroupLeader)); } else Group.addPartyMember(ep._player); } }
	 * catch(Exception ex){ if(team._team != null && team._team._player != null )
	 * team._team._player.sendMessage("Error on league creation");
	 * ex.printStackTrace(); } }
	 * 
	 * public static final void expelFromGroup(CQFDEventTeam team, Player pl){ try{
	 * 
	 * if(pl.getParty() != null) if(pl.getParty().getMemberCount() == 2)
	 * pl.getParty().dissolveParty(); else pl.getParty().removePartyMember(pl,
	 * true);
	 * 
	 * } catch(Exception ex){ if(team._team != null && team._team._player != null &&
	 * pl != null) pl.sendMessage("Error on group expel of : "+ pl.getName());
	 * ex.printStackTrace(); } }
	 */
	 ...
	 * player.teleToLocation(new Location(loc._x, loc._y, loc._z, loc._head),
	 * loc._instanceId); }
	 */
	...
	public static void teleportPlayer(CQFDEventTeam team, CQFDEventPlayer ep, CQFDEventPlayer loc) {
		...
		TeleportService.teleportTo(ep._player, loc._world, loc._instanceId, loc._x, loc._y, loc._z, loc._head, 0, true);
	}
	...
	public String printHtmlEventStat(CQFDEvent event) {
		...
		for (Integer rewardId : event._rewards.keySet()) {
			...
			for (CQFDReward reward : event._rewards.get(rewardId)) {
				...
				// ItemHolder.getInstance().getTemplate(reward.getItemId()); // L2
