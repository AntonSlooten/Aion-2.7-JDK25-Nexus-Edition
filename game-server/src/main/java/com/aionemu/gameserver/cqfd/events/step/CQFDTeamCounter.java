...
public class CQFDTeamCounter implements CQFDListener {
	...
	// LoggerFactory.getLogger(CQFDTeamCounter.class);

	public CQFDTeamCounter(CQFDEvent event, CQFDEventTeam team, CQFDListenerType lisenerType, CQFDORDER order, int init,
			int stepId, CQFDCOUNTERTYPE type) {
		_event = event;
		_team = team;
	...
	public CQFDTeamCounter(CQFDEvent event, CQFDEventTeam team, CQFDListenerType lisenerType,
			Class<? extends CQFDEventTask> c, int eatch, CQFDORDER order, int init, int stepId, CQFDCOUNTERTYPE type) {
		_event = event;
		_team = team;
		_lisenerType = lisenerType;
		_actionC = c;
		...
		case INC:
			...
			if (_eatch != 0) {
				if (++_actionIndex >= _eatch) {
					_actionIndex = 0;
					try {
						_actionC.getDeclaredConstructor(CQFDEventTeam.class, Object[].class).newInstance(_event, params)
								.run();
					} catch (Exception e) {
