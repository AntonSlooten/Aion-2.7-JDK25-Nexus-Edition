...
public class CustomPlayerRank {
	...
	private static final HashMap<Integer, BossInfo> BOSS_PTS = new HashMap<>();
	...
	private static final Integer[] ELITE_LIST = { 215488// Celestius
	};
	...
	public void checkLvUp() {
		...
		_currentRank++;
		_currentPts = 0;
		PacketSendUtility.broadcastPacket(_player, new SM_LEVEL_UPDATE(_player.getObjectId(), 0, _player.getLevel()),
				true);
		PacketSendUtility.sendMessage(_player, "Rank up !");
		// PacketSendUtility.broadcastPacket(_player, new SM_PLAYER_INFO(_player, true),
		// true);
	}
	...
	public static synchronized void onMonsterKill(Creature creature) {
		if (creature instanceof Npc) {
			...
			if (containElite(npc.getNpcId()) || npc.isBoss() && BOSS_LIST.contains(npc.getNpcId())) {// si est un boss
																										// et qui donne
																										// des pts
