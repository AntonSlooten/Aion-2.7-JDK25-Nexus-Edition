...
public class CmdTvt2 extends BaseCommand {
	...
	/*
	 * syntax \\tvt2 <info> <start> <reg> <unreg> syntax \\tvt2 <start> <id> <time
	 * (18 00 hours minutes)
	 */
	...
	public void execute(Player player, String... params) {
		...
		if (params[0].equals("info")) {
			...
			PacketSendUtility.sendMessage(player,
					"CronService add tvt2 event shedule at: " + params[2] + ":" + params[3]);
			log.info("[TVT] Scheduled tvt of id " + l.getId() + " based on cron expression: " + tvtTime);
		} else if (params[0].equals("reg")) {
			PacketSendUtility.sendPacket(player, new SM_QUEST_ACTION(4, 0,
					TvtService.getInstance().getTvtByLevel(player.getLevel()).getRemainingTime()));
