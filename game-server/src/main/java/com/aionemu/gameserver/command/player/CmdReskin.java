...
public class CmdReskin extends BaseCommand {
	...
	public void execute(Player player, String... params) {
		...
		if (inventory.countItemsById(extractItemId) > 1) {
			PacketSendUtility.sendBrightYellowMessage(player, "[Reskin] L'objet [item:" + extractItemId
					+ "] est pr\u00E9sent plusieurs fois dans votre inventaire. Il ne doit y en avoir qu'un.");
			return;
		}
		...
		if (extractCat != ItemCategory.HELMET && extractCat != ItemCategory.GLOVES && extractCat != ItemCategory.JACKET
				&& extractCat != ItemCategory.PANTS && extractCat != ItemCategory.SHOES
				&& extractCat != ItemCategory.SHOULDERS) {
			PacketSendUtility.sendBrightYellowMessage(player,
					"[Reskin] Veuillez reskin votre vêtement sur une armure avant de faire la commande.");
			return;
		}
		...
		if (keepItem.getItemTemplate().getEquipmentType() != EquipType.ARMOR) {
			PacketSendUtility.sendBrightYellowMessage(player,
					"[Reskin] Seul les armures sont reskinable via cette command");
			return;
		}
		...
		if (tolls < cost && !player.isGM()) {
			PacketSendUtility.sendBrightYellowMessage(player,
					"[Reskin] Vous n'avez pas assez de points (" + tolls + "/" + cost + ")");
			return;
		}
		...
		PacketSendUtility.sendBrightYellowMessage(player,
				"[Reskin] Votre item est reskin. R\u00E9\u00E9quiper le pour le voir.");
