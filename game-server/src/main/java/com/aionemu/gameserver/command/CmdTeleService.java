	...
	public void loadTeleTemplate() {
		...
		try {
			...
			while (resultSet.next()) {
				teleTemplate.put(resultSet.getString("name"), new CmdTeleTemplate(resultSet.getInt("worldid"),
						resultSet.getFloat("x"), resultSet.getFloat("y"), resultSet.getFloat("z")));
			}
	...
	public boolean addTeleTemplate(String name, WorldPosition pos) {
		...
		try {
			...
			PreparedStatement stmt = con
					.prepareStatement("INSERT INTO teleport (name, worldid, x, y, z) VALUES (?, ?, ?, ?, ?);");
