		...
		if (params[0].equalsIgnoreCase("disband")) {
			...
			if (level < 1 || level > 5) {
				...
				PacketSendUtility.sendMessage(admin,
						"legion " + legion.getLegionName() + " is already with that level.");
				return;
			}
			...
			PacketSendUtility.sendMessage(admin,
					"legion " + legion.getLegionName() + " has raised from " + old + " to " + level + " level.");
			...
			PacketSendUtility.sendMessage(admin, "legion " + legion.getLegionName() + " has raised from " + old + " to "
					+ points + " contributiong points.");
			...
			PacketSendUtility.sendMessage(admin,
					"legion " + old + " has changed name from " + old + " to " + NewlegionName + ".");
			...
			FastList<String> message = FastList.newInstance(), online = FastList.newInstance(),
					offline = FastList.newInstance();
			...
			for (int memberId : members) {
				...
				if (pl != null) {
					...
					offline.add(
							pcd.getName() + " (lv" + pcd.getLevel() + ") classId " + pcd.getPlayerClass().getClassId());
				}
			}
			...
			if (player.getLegionMember().getRank() == LegionRank.BRIGADE_GENERAL) {
				PacketSendUtility.sendMessage(admin,
						"player " + player.getName() + " is a brigade general. Disband legion!");
				return;
			}
			...
			if (service.removePlayerFromLegionAsItself(player)) {
				...
				PacketSendUtility.sendMessage(admin,
						"You have failed to kick player " + player.getName() + " from legion.");
			}
			...
			if (target.isLegionMember()) {
				PacketSendUtility.sendMessage(admin, "player " + target.getName() + " is a already member of "
						+ target.getLegion().getLegionName() + "!");
				return;
			}

			if (service.directAddPlayer(legion, target)) {
				PacketSendUtility.sendMessage(admin,
						"player " + target.getName() + " was added to " + legion.getLegionName());
			} else {
			...
			if (!legion.isMember(target.getObjectId())) {
				PacketSendUtility.sendMessage(admin, "player " + target.getName() + " is not a member of "
						+ legion.getLegionName() + ", invite them!");
				return;
			}
			...
			PacketSendUtility.sendMessage(admin,
					"You have sucessfully demoted " + bgplayer.getName() + " to Legionary rank.");
			...
			if (params[2].equalsIgnoreCase("centurion")) {
				...
				PacketSendUtility.broadcastPacketToLegion(target.getLegion(),
						new SM_LEGION_UPDATE_MEMBER(target, 0, ""));
