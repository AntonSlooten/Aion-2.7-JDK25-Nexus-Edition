	...
	public void execute(final Player player, String... params) {
		...
		if (extractItemId == 0) {
			PacketSendUtility.sendBrightYellowMessage(player,
					"[Remodel] " + params[1] + " n'est pas un ID d'objet valide.");
			return;
		}
		...
		if (inventory.countItemsById(keepItemId) > 1) {
			PacketSendUtility.sendBrightYellowMessage(player, "[Remodel] L'objet [item:" + keepItemId
		...
		if (templateExtractItem == null) {
			PacketSendUtility.sendBrightYellowMessage(player,
					"[Remodel] L'objet " + extractItemId + " n'existe pas dans le client 2.7");
			return;
		}

		if (keepItem == null) {
			PacketSendUtility.sendBrightYellowMessage(player, "[Remodel] L'objet [item:" + keepItemId
					+ "] doit \u00EAtre dans votre inventaire et non \u00E9quipp\u00E9.");
			return;
		}

		// Check that types match.
		if (keepItem.getItemTemplate().getWeaponType() != templateExtractItem.getWeaponType()
				|| (templateExtractItem.getArmorType() != ArmorType.CLOTHES
						&& keepItem.getItemTemplate().getArmorType() != templateExtractItem.getArmorType())
			...
			PacketSendUtility.sendPacket(player,
					SM_SYSTEM_MESSAGE.STR_CHANGE_ITEM_SKIN_NOT_COMPATIBLE(
							...
							new DescriptionId(templateExtractItem.getNameId())));
			return;
		}
		...
		if (tolls < tollPrice) {
			PacketSendUtility.sendBrightYellowMessage(player,
					"[Remodel] Vous n'avez pas assez de points (" + tolls + "/" + tollPrice + ")");
			return;
		}
		...
		PacketSendUtility.sendBrightYellowMessage(player,
				"[Remodel] Votre objet " + params[0] + " vient d'\u00EAtre remodel\u00E9 en [item:" + params[1] + "].");
