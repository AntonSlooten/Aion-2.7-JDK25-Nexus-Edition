...
public class CmdPowerUp extends BaseCommand {
	...
	public void execute(Player admin, String... params) {
		...
		// PacketSendUtility.sendMessage(admin, "Syntax: //powerup [playerName]
		// [Multiplier = 2]\n"
		...
		// player.getLifeStats().increaseHp(TYPE.HP, admin.getLifeStats().getMaxHp() +
		// 1);
		// player.getLifeStats().increaseMp(TYPE.MP, admin.getLifeStats().getMaxMp() +
		// 1);
		// PacketSendUtility.sendPacket(player, new SM_STATS_INFO(admin));
		// if (player == admin)
		// PacketSendUtility.sendMessage(player, "You are now normal again.");
		// else {
		// PacketSendUtility.sendMessage(admin, "Player " + player.getName() + " is now
		// normal again.");
		// PacketSendUtility.sendMessage(player, "Admin " + admin.getName() + " made you
		// normal again.");
		// }
		// return;
		// }
		// player.getGameStats().setStat(StatEnum.MAXMP, admin.getLifeStats().getMaxHp()
		// * index);
		// player.getGameStats().setStat(StatEnum.MAXHP, admin.getLifeStats().getMaxMp()
		// * index);
		//
		...
		// admin.getGameStats().getStatBonus(StatEnum.BOOST_MAGICAL_SKILL) * index *
		// 15);
		//
		...
		// PacketSendUtility.sendPacket(player, new SM_STATS_INFO(admin));
		// if (player == admin)
		// PacketSendUtility.sendMessage(player, "You are now " + index + " times more
