...
public class CmdMultipleSpawn extends BaseCommand {
	/*
	 * linex/y id npcRadius MaxRang hcyl id npcRadius MaxRang cyl id npcRadius
	 * MaxRang star id npcRadius MaxRang nbPointed square id npcRadius MaxRang rec
	 * id npcRadius Height Length
	 *
	 *
	 * exemple: - cyl -- //ms cyl 700454 2 30 ---
	 * http://gyazo.com/bb37563de27c13d2a2559af5a1c20402
	 *
	 * - Star -- //ms star 700454 4 10 50 ---
	 * http://gyazo.com/71666250ddb98de286b2804da4ab9d5a
	 *
	 * - Square -- //ms square 700454 4 20 ---
	...
	private final static String help = "============ MultipleSpawn ============= <br>"
			+ "- linex/y id npcRadius MaxRang          <br>" + "- hcyl    id npcRadius MaxRang          <br>"
			+ "- cyl     id npcRadius MaxRang          <br>" + "- star    id npcRadius nbPointed MaxRang<br>"
			+ "- square  id npcRadius MaxRang          <br>" + "- rec     id npcRadius Height Length    <br>"
	...
	public void execute(Player admin, String... params) {
		...
		if (params[0].equalsIgnoreCase("hcyl")) {
			...
			npcs = star(admin, Integer.parseInt(params[1]), Integer.parseInt(params[2]), Integer.parseInt(params[3]),
			...
			npcs = rec(admin, Integer.parseInt(params[1]), Integer.parseInt(params[2]), Integer.parseInt(params[3]),
					Integer.parseInt(params[4]));
	...
	public ArrayList<VisibleObject> rec(Player admin, int npcId, int npcRadius, int Height, int Length) {
		ArrayList<VisibleObject> npcs = new ArrayList<>();
		for (int xOff = -Height / 2; xOff <= Height / 2; xOff = xOff + npcRadius) {
			for (int yOff = -Length / 2; yOff <= Length / 2; yOff = yOff + npcRadius) {
				npcs.add(SpawnEngine.spawnObject(SpawnEngine.addNewSingleTimeSpawn(admin.getWorldId(), npcId,
						admin.getX() + xOff, admin.getY() + yOff, admin.getZ(), admin.getHeading()),
						admin.getInstanceId()));
			}
		}
		return npcs;
	...
	public ArrayList<VisibleObject> yLine(Player admin, int npcId, int npcRadius, int MaxRang) {
		...
		for (int yOff = 0; yOff <= MaxRang; yOff = yOff + npcRadius) {
			npcs.add(SpawnEngine.spawnObject(SpawnEngine.addNewSingleTimeSpawn(admin.getWorldId(), npcId, admin.getX(),
					admin.getY() + yOff, admin.getZ(), admin.getHeading()), admin.getInstanceId()));
		}
		return npcs;
	}
	...
	public ArrayList<VisibleObject> xLine(Player admin, int npcId, int npcRadius, int MaxRang) {
		...
		for (int xOff = 0; xOff <= MaxRang; xOff = xOff + npcRadius) {
			npcs.add(SpawnEngine.spawnObject(SpawnEngine.addNewSingleTimeSpawn(admin.getWorldId(), npcId,
					admin.getX() + xOff, admin.getY(), admin.getZ(), admin.getHeading()), admin.getInstanceId()));
		}
		return npcs;
	}
	...
	public ArrayList<VisibleObject> hcyl(Player admin, int npcId, int npcRadius, int MaxRang) {
		...
		for (double i = 0; i < 2 * pi; i = i + baseAngle) {
			npcs.add(SpawnEngine.spawnObject(
					SpawnEngine.addNewSingleTimeSpawn(admin.getWorldId(), npcId,
							(float) (admin.getX() + Math.cos(i) * MaxRang),
							(float) (admin.getY() + Math.sin(i) * MaxRang), admin.getZ(), admin.getHeading()),
					admin.getInstanceId()));
		}
		return npcs;
	}
