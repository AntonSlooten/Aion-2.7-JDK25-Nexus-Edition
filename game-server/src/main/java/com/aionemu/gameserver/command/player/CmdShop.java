...
/**
 * @author Ferosia Warning! Requires additional table. See
 *         sql/updates/addWebShop.sql
 */
...
public class CmdShop extends BaseCommand {
	...
	public void execute(final Player player, String... params) {
				...
				public void handleRead(ResultSet rset) throws SQLException {
					...
					while (rset.next()) {
						...
						if (removeShopTransaction(transactionId, playerId)) {
							...
							String yourCommand = TranslationService.SHOP_MAIL_TITLE.toString(player); // Here is your
																										// command
							String thankYou = TranslationService.SHOP_THANKYOU_ORDER.toString(player); // Thank you for
																										// your order
							...
							if (nb > maxStackCount && maxStackCount != 0 && item != 182400001) {
								...
								for (int i = 1; i <= nb; i += maxStackCount) {
									...
									// TODO: if(!player.getMailbox().haveFreeSlots()) => create new pending shop
									// transaction
									SystemMailService.getInstance().sendMail("WebShop", playerName,
											itemTemplate.getName(), yourCommand +
									// One mail was not enough, please check your mails
													multipleMail + thankYou,
											item, qtySend, 0, true);
								}
							} else if (item == 182400001) {
								SystemMailService.getInstance().sendMail("WebShop", playerName, itemTemplate.getName(),
										yourCommand + thankYou, 0, -1, nb, true);
							} else {
								SystemMailService.getInstance().sendMail("WebShop", playerName, itemTemplate.getName(),
										yourCommand + thankYou, item, nb, 0, true);
							}
						} else {
					...
					if (count == 0) {
						...
						String message = TranslationService.SHOP_ORDERS_RECEIVED.toString(player, String.valueOf(count),
								playerName);
						sendCommandMessage(player, message);
					}
				}
		...
		// Log error and inform player if there is a database error
			...
			log.error("[Shop][Error] Could not catch transaction list for player " + playerId + " from DB: "
					+ ex.getMessage(), ex);
			String message = TranslationService.GENERAL_ERROR_DB.toString(player);
			sendCommandMessage(player, message);
			return;
	...
	// Function to remove transaction from database. Must be done first for security
	// reasons.
	public boolean removeShopTransaction(int transactionId, int playerId) {
		...
		try {
			...
			log.error("[Shop][Error] Could not delete transaction " + transactionId + " for player " + playerId
