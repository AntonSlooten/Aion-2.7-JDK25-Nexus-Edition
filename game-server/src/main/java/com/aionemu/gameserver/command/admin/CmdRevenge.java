...
public class CmdRevenge extends BaseCommand {
	...
	public void execute(final Player admin, String... params) {
		...
		if (requested) {
			PacketSendUtility.sendPacket(player, new SM_QUESTION_WINDOW(902247, 0, message));
			return;
		}
	}
	...
	private void start(final Player admin, final Player player) {
		...
		MessagerAddition.announce(player,
				"[Revenge]: У вас есть 15 секунд на подготовку к битве!\nЧерез 15 секунд вы будете телепортированы к врагу!");
		ThreadPoolManager.getInstance().schedule(new Runnable() {

			@Override
			public void run() {
				TeleportService.teleportTo(admin, player.getWorldId(), player.getX(), player.getY(), player.getZ(),
						3000, true);
				...
				if (player.getLifeStats().isAlreadyDead()) {
					...
					adminscore += 1;
					MessagerAddition.announce(player,
							"[Revenge]Вы Проиграли В битве! Игрок" + admin.getName() + "Получает одно Очко");
					MessagerAddition.announce(admin,
							"[Revenge]Поздравляю!Вы Выйграли,вы получаете Одно очко в свой счет");
					admIsWin1 = true;
					restore(player);
					restore(admin);
					TeleportService.teleportTo(player, 510010000, 256, 256, 49, 3000, true);
					...
					PlayerReviveService.revive(player, 100, 100, false);
					MessagerAddition.announce(player,
							"[Revenge]Ни один не выйграл,никто не получает дополнительное очко");
					MessagerAddition.announce(admin,
							"[Revenge]Ни один не выйграл,никто не получает дополнительное очко");
					isDraw1 = true;
					restore(player);
					...
					playerscore += 1;
					MessagerAddition.announce(player,
							"[Revenge]Поздравляю Вы Выйграли!Вы получаете одно дополнительное очко!");
					MessagerAddition.announce(admin,
							"[Revenge]Вы Проиграли,ваш противник получает дополнительное очко!");
					playerIsWin1 = true;
					restore(player);
					restore(admin);
					TeleportService.teleportTo(player, 510010000, 256, 256, 49, 3000, true);
				...
				if (player.getWorldId() != 510010000) {
					MessagerAddition.announceAll(
							"[Revenge]: Игрок" + player.getName() + "Ушел с поля битвы!\nВыигрывает " + admin.getName(),
							0);
				}
				if (admin.getWorldId() != 510010000) {
					MessagerAddition.announceAll(
							"[Revenge]: Игрок" + admin.getName() + "Ушел с поля битвы!\nВыигрывает " + player.getName(),
							0);
	...
	private void onEnd(final Player player, final Player admin) {
		MessagerAddition.announceAll("[Revenge]:И так результаты битвы " + admin.getName() + "vs" + player.getName(),
				0);
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				if (adminscore == playerscore) {
					MessagerAddition.announceAll("[Revenge]: И так результаты битвы " + admin.getName() + "vs"
							+ player.getName() + " <НИЧЬЯ> !", 0);
				}
				if (adminscore > playerscore) {
					MessagerAddition.announceAll("[Revenge]: И так результаты битвы " + admin.getName() + "vs"
							+ player.getName() + " - " + admin.getName() + " Выигрывает!", 0);
				} else {
					MessagerAddition.announceAll("[Revenge]: И так результаты битвы " + admin.getName() + "vs"
							+ player.getName() + " - " + player.getName() + " Выигрывает!", 0);
				}
