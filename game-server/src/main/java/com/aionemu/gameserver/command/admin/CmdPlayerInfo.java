	...
	public void execute(Player admin, String... params) {
		...
		if (params[1].equals("item")) {
			...
			for (PlayerSkillEntry element : sle) {
				strbld.append("#" + element.getSkillId() + " : " + element.getSkillName() + " au level "
						+ element.getSkillLevel() + "\n");
			}
			showAllLines(admin, strbld.toString());
			...
			if (legion == null) {
				...
				strbld.append("-legion info:\n  name: " + legion.getLegionName() + ", level: " + legion.getLegionLevel()
						+ "\n  members(online):\n");
				for (LegionMemberEx act : legionmemblist) {
					strbld.append("    " + act.getName() + "(" + (act.isOnline() ? "online" : "offline") + ")"
