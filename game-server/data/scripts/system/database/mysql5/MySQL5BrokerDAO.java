		...
		DB.select("SELECT * FROM broker", new ReadStH() {
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					brokerItems.add(new BrokerItem(item, itemId, itemPointer, itemCount, itemCreator, price, seller,
							sellerId, itemBrokerRace, isSold, isSettled, expireTime, settleTime));
				}
			}
		});
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					brokerItems.add(new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime,
							activationCount, false, false, slot, location, enchant, itemSkin, fusionedItem,
