...
public class MySQL5PetitionDAO extends PetitionDAO {
	...
	public Petition getPetitionById(int petitionId) {
		...
		try {
			...
			result = new Petition(rset.getInt("id"), rset.getInt("player_id"), rset.getInt("type"),
					rset.getString("title"), rset.getString("message"), rset.getString("add_data"),
					status.getElementId());
	...
	public void deletePetition(int playerObjId) {
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement(
					"DELETE FROM petitions WHERE player_id = ? AND (status = 'PENDING' OR status='IN_PROGRESS')");
