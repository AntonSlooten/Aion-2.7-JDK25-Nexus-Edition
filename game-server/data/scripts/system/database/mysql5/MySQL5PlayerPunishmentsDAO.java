...
public class MySQL5PlayerPunishmentsDAO extends PlayerPunishmentsDAO {
		...
		DB.insertUpdate(UPDATE_QUERY, new IUStH() {
			...
			public void handleInsertUpdate(PreparedStatement ps) throws SQLException {
				if (punishmentType == PunishmentType.PRISON) {
					...
					ps.setLong(1,
							(player.getGatherableTimer() - (System.currentTimeMillis() - player.getStopGatherable()))
									/ 1000);
				}
	...
	@Override
	public void punishPlayer(final int playerId, final PunishmentType punishmentType, final long duration,
			final String reason) {
	...
	public CharacterBanInfo getCharBanInfo(final int playerId) {
		...
		DB.select(SELECT_QUERY, new ParamReadStH() {
			...
			public void handleRead(ResultSet rs) throws SQLException {
				while (rs.next()) {
					charBan[0] = new CharacterBanInfo(playerId, rs.getLong("start_time"), rs.getLong("duration"),
							rs.getString("reason"));
				}
			}
		});
