...
public class MySQL5PlayerBindPointDAO extends PlayerBindPointDAO {
	...
	public boolean insertBindPoint(Player player) {
		...
		try {
			...
			log.error("Could not store BindPointPosition data for player " + player.getObjectId() + " from DB: "
					+ e.getMessage(), e);
			return false;
		} finally {
			DatabaseFactory.close(con);
	...
	public boolean updateBindPoint(Player player) {
		...
		try {
			...
			log.error("Could not update BindPointPosition data for player " + player.getObjectId() + " from DB: "
					+ e.getMessage(), e);
			return false;
		} finally {
			DatabaseFactory.close(con);
