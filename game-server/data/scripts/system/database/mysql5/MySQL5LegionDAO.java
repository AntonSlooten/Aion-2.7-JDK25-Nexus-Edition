	...
	public void storeLegion(final Legion legion) {
		DB.insertUpdate(UPDATE_LEGION_QUERY, new IUStH() {
			...
			public void handleInsertUpdate(PreparedStatement stmt) throws SQLException {
				log.debug(
						"[DAO: MySQL5LegionDAO] storing player " + legion.getLegionId() + " " + legion.getLegionName());
		...
		boolean success = DB.select(SELECT_LEGION_QUERY2, new ParamReadStH() {
			...
			public void handleRead(ResultSet resultSet) throws SQLException {
				while (resultSet.next()) {
					...
					legion.setLegionPermissions(resultSet.getShort("deputy_permission"),
							resultSet.getShort("centurion_permission"), resultSet.getShort("legionary_permission"),
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					Item item = new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime,
							activationCount, isEquiped == 1, false, slot, storage, enchant, itemSkin, fusionedItem,
