	...
	public FastMap<Integer, IGItem> loadInGameShopItems() {
		...
		try {
			...
			while (rset.next()) {
				...
				items.put(objectId,
						new IGItem(objectId, itemId, itemCount, itemPrice, category, list, salesRanking, description));
			}
			rset.close();
			stmt.close();
		} catch (Exception e) {
	...
	public void saveIngameShopItem(int objectId, int itemId, int itemCount, int itemPrice, int category, int list,
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement(
					"INSERT INTO ingameshop(object_id, item_id, item_count, item_price, category, list, sales_ranking, description)"
							+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
