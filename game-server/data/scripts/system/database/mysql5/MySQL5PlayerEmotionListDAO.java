...
public class MySQL5PlayerEmotionListDAO extends PlayerEmotionListDAO {
	 ...
	 * 
	 * @see
	 * com.aionemu.gameserver.dao.PlayerEmotionListDAO#loadEmotions(com.aionemu.
	...
	public void loadEmotions(Player player) {
		...
		try {
			...
			log.error("Could not restore emotionId for playerObjId: " + player.getObjectId() + " from DB: "
					+ e.getMessage(), e);
		} finally {
	 ...
	 * 
	 * @see
	 * com.aionemu.gameserver.dao.PlayerEmotionListDAO#insertEmotion(com.aionemu.
	 * gameserver.model.gameobjects.player.Player , int)
	 */
	@Override
	public void insertEmotion(Player player, Emotion emotion) {
		...
		try {
			...
			log.error("Could not store emotionId for player " + player.getObjectId() + " from DB: " + e.getMessage(),
					e);
		} finally {
			DatabaseFactory.close(con);
		}
