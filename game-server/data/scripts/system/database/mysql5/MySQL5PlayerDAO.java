...
public class MySQL5PlayerDAO extends PlayerDAO {
	...
	public void storePlayer(final Player player) {
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement(
					"UPDATE players SET name=?, exp=?, recoverexp=?, x=?, y=?, z=?, heading=?, world_id=?, gender=?, race=?, player_class=?, last_online=?, quest_expands=?, npc_expands=?, advenced_stigma_slot_size=?, warehouse_size=?, note=?, title_id=?, dp=?, soul_sickness=?, mailbox_letters=?, reposte_energy=?, mentor_flag_time=?, unstuck=?, gmconfig=? WHERE id=?");
	...
	public PlayerCommonData loadPlayerCommonData(final int playerObjId) {
		...
		try {
			...
			if (resultSet.next()) {
				...
				PreparedStatement stmtLang = con
						.prepareStatement("SELECT `locale` FROM `account_locale` WHERE `account_id`=?");
				...
				if (rsetLang.next()) {
					...
					log.error("There were a locale website error for account " + accountId
							+ ". Set default locale as fr");
					cd.setLocale("fr");
				}
	...
	public long getCreationTime(final int objectId) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("SELECT creation_date FROM `players` WHERE `id` = ?");
	...
	public void storePlayerName(final PlayerCommonData recipientCommonData) {
		...
		try {
			...
			log.error("Error saving playerName: " + recipientCommonData.getPlayerObjId() + " "
