...
public class MySQL5PlayerLifeStatsDAO extends PlayerLifeStatsDAO {
	...
	public void insertPlayerLifeStat(final Player player) {
		...
		try {
			...
			log.error("Could not store PlayerLifeStat data for player " + player.getObjectId() + " from DB: "
					+ e.getMessage(), e);
		} finally {
			DatabaseFactory.close(con);
		}
	...
	public void updatePlayerLifeStat(final Player player) {
		...
		try {
			...
			log.error("Could not update PlayerLifeStat data for player " + player.getObjectId() + " from DB: "
					+ e.getMessage(), e);
		} finally {
			DatabaseFactory.close(con);
		}
