	...
	public QuestStateList load(final Player player) {
		...
		try {
			...
			while (rset.next()) {
				...
				QuestState questState = new QuestState(questId, status, questVars, questVars2, completeCount,
			...
			log.error("Could not restore QuestStateList data for player: " + player.getObjectId() + " from DB: "
					+ e.getMessage(), e);
		} finally {
