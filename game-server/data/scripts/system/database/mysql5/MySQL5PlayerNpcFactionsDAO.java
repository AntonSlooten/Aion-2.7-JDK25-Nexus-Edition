...
public class MySQL5PlayerNpcFactionsDAO extends PlayerNpcFactionsDAO {
	...
	public void loadNpcFactions(Player player) {
		...
		try {
			...
			log.error("Could not restore Npc faction data for playerObjId: " + player.getObjectId() + " from DB: "
					+ e.getMessage(), e);
		} finally {
	...
	private void insertNpcFaction(int playerObjectId, NpcFaction faction) {
		...
		try {
			...
			log.error("Could not insert Npc faction data for playerObjId: " + playerObjectId + " from DB: "
					+ e.getMessage(), e);
		} finally {
			DatabaseFactory.close(con);
		}
	...
	private void updateNpcFaction(int playerObjectId, NpcFaction faction) {
		...
		try {
			...
			log.error("Could not update Npc faction data for playerObjId: " + playerObjectId + " from DB: "
					+ e.getMessage(), e);
		} finally {
			DatabaseFactory.close(con);
		}
