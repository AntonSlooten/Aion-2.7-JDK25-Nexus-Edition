...
public class TvTInstance extends GeneralInstanceHandler {
	...
	public boolean isEnemyPlayer(Creature effector, Creature effected) {
		return ((Npc) effector).getObjectTemplate().getRace() != ((Player) effected).getRace()
				&& !((Player) effected).isAggroIconTo(null); // Dont Know!Maybe effected, but method dont compatible
	}
	...
	public void onEnterInstance(Player player) {
		...
		// PacketSendUtility.sendSysMessage(player, "" + ((getRemainingTime() / 60) +
		// 1));
		MessagerAddition.announce(player,
				"[Ascension]Tvt: Welcome to Tvt!Event over in " + ((getRemainingTime() / 60) + 1) + "min.");
	...
	public void onLeaveInstance(Player player) {
		if (TvtService.getInstance().getTvtByLevel(player.getLevel()).getIsStarted()) {
			...
			// LanguageHandler.translateRU(CustomMessageIdRU.TVT_LEAVE));
			...
			for (TvtSchedule.TvtBoss boss : l.getTvtBoss()) {
				if (getPoint(player.getRace()) == boss.getNeedScore()) {
					for (String p : boss.getLoc()) {
						...
						// SpawnTemplate bossT = SpawnEngine.addNewSpawn(mapId, instanceId,
						// boss.getBossId(), x, y, z, head, 0, 0, true);
						...
						TvtBossTemplate bossTmp = new TvtBossTemplate(boss.getReward(), boss.getVote(), boss.isCd(),
								boss.getAp(), boss.isBoost(), npc);
	...
	/*
	 * private void spawnClock(Player player) { TvtRegistrator tvt =
	 * TvtService.getInstance().getTvtByLevel(player.getLevel()); TvtSchedule
	 * tvtSchedule = TvtService.getInstance().getTvtSchedule(); for
	 * (TvtSchedule.TvtLevel l : tvtSchedule.getTvtLevelList()) { if (!l.getBoss())
	 * { continue; } if (l.getId() != tvt.getTvtId()) { continue; }
	 * 
	 * for (TvtSchedule.TvtClock c : l.getTvtClock()) { if (player.getRace() !=
	 * c.getRace()) { for (String p : c.getLoc()) { StringTokenizer st = new
	 * StringTokenizer(p, ";"); if (st.countTokens() != 4) { continue; } float x =
	 * Float.parseFloat(st.nextToken()); float y = Float.parseFloat(st.nextToken());
	 * float z = Float.parseFloat(st.nextToken()); byte head =
	 * Byte.parseByte(st.nextToken()); // SpawnTemplate clock =
	 * SpawnEngine.addNewSpawn(mapId, instanceId, c.getClockId(), x, y, z, head, 0,
	 * 0, true); SpawnTemplate clock = SpawnEngine.addNewSpawn(mapId,
	 * c.getClockId(), x, y, z, head, 0); SpawnEngine.spawnObject(clock,
	 * instanceId); } } } } }
	 */
			...
			for (TvtSchedule.TvtHelpers h : l.getTvtHelpers()) {
				if (h.getRace().equals(race)) {
					for (String p : h.getLoc()) {
						...
						float z = Float.parseFloat(st.nextToken());
						byte head = Byte.parseByte(st.nextToken());
						// SpawnTemplate helper = SpawnEngine.addNewSpawn(mapId, instanceId,
						// h.getHelperId(), x, y, z, head, 0, 0, true);
	...
	private void mail(Player player) {
		...
		// SystemMailService.getInstance().sendMail(service, player.getName(),
		// senderName, message, 0, 1, 0, false, "");
		...
		// DAOManager.getDAO(PlayerDAO.class).setMoney(player.getCommonData().getMoney()
		// + 1, player.getObjectId());
		// player.getCommonData().setMoney(player.getCommonData().getMoney() + 1);
		// PacketSendUtility.sendPacket(player, new
		// SM_INGAMESHOP_BALANCE(player.getCommonData().getMoney()));
		...
		for (Player pl : instance.getPlayersInside()) {
			if (pl.getRace() == Race.ASMODIANS) {
				PacketSendUtility.sendMessage(pl,
						"Elysea:" + getPoint(Race.ELYOS) + "| Asmodea:" + getPoint(Race.ASMODIANS));
			} else {
				PacketSendUtility.sendMessage(pl,
						"Asmodea:" + getPoint(Race.ASMODIANS) + "| Elysea:" + getPoint(Race.ELYOS));
			}
		}
	}
