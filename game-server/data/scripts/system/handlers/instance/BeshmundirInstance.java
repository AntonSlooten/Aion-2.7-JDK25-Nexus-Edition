...
public class BeshmundirInstance extends GeneralInstanceHandler {
	...
	public void onDie(Npc npc) {
		...
		case 216586: // Spawn macumbelo
			...
			if (macunbello < 12) {
				npcMacunbello = (Npc) spawn(216734, 981.015015f, 134.373001f, 241.755005f, (byte) 30); // strongest
																										// macunbello
				SkillEngine.getInstance().applyEffectDirectly(19048, npcMacunbello, npcMacunbello, 0);
			} else if (macunbello < 14) {
				npcMacunbello = (Npc) spawn(216737, 981.015015f, 134.373001f, 241.755005f, (byte) 30); // 2th strongest
																										// macunbello
				SkillEngine.getInstance().applyEffectDirectly(19047, npcMacunbello, npcMacunbello, 0);
			} else if (macunbello < 21) {
				npcMacunbello = (Npc) spawn(216736, 981.015015f, 134.373001f, 241.755005f, (byte) 30); // 2th weakest
																										// macunbello
	...
	public void onEnterZone(Player player, ZoneInstance zone) {
		if (zone.getAreaTemplate().getZoneName() == ZoneName.COURTYARD_OF_ETERNITY_300170000
				&& CustomFun.ISBARYA_CUSTOM) {
			PacketSendUtility.sendYellowMessageOnCenter(player,
					"The power of isbariya depends on his loyal gardiens !");
		}
	}
