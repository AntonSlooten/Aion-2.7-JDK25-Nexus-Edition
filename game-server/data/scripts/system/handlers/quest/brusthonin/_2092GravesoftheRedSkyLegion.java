...
/**
 *
 * Fixed for AXE by D3x Adapted by Aion Phenix for Lightning 2.7 Improved by
 * Metos
 *
 */
public class _2092GravesoftheRedSkyLegion extends QuestHandler {
	...
	private final static int[] npc_ids = { 205150, 205188, 700394, 205190, 730156, 730158, 730159, 730160, 730161,
			730162, 730163, 205208, 205209, 205210, 205211, 205212, 205213, 205214 };
		...
		if (targetId == 205150) {
			...
			if (var == 5) {
				if (!isAlreadyTalk(qs.getQuestVar2(), 0) && (spawn_Surdi == null || !spawn_Surdi.isSpawned())) {
					spawn_Surdi = QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 205208,
							npc.getX(), npc.getY(), npc.getZ(), npc.getHeading()); // Sudri's Ghost
				}
				return true;
			}
			...
			if (var == 5) {
				if (!isAlreadyTalk(qs.getQuestVar2(), 1) && (spawn_Angeiya == null || !spawn_Angeiya.isSpawned())) {
					spawn_Angeiya = QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 205209,
							npc.getX(), npc.getY(), npc.getZ(), npc.getHeading()); // Angeiya's Ghost
				}
				return true;
			}
			...
			if (var == 5) {
				if (!isAlreadyTalk(qs.getQuestVar2(), 2) && (spawn_Erna == null || !spawn_Erna.isSpawned())) {
					spawn_Erna = QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 205210,
							npc.getX(), npc.getY(), npc.getZ(), npc.getHeading()); // Erna's Ghost
				}
				return true;
			}
			...
			if (var == 5) {
				if (!isAlreadyTalk(qs.getQuestVar2(), 3) && (spawn_Genta == null || !spawn_Genta.isSpawned())) {
					spawn_Genta = QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 205211,
							npc.getX(), npc.getY(), npc.getZ(), npc.getHeading()); // Genta's Ghost
				}
				return true;
			}
			...
			if (var == 5) {
				if (!isAlreadyTalk(qs.getQuestVar2(), 4) && (spawn_Sith == null || !spawn_Sith.isSpawned())) {
					spawn_Sith = QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 205212,
							npc.getX(), npc.getY(), npc.getZ(), npc.getHeading()); // Sith's Ghost
				}
				return true;
			}
			...
			if (var == 5) {
				if (!isAlreadyTalk(qs.getQuestVar2(), 5) && (spawn_Barache == null || !spawn_Barache.isSpawned())) {
					spawn_Barache = QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 205213,
							npc.getX(), npc.getY(), npc.getZ(), npc.getHeading()); // Barache's Ghost
				}
				return true;
			}
			...
			if (var == 5) {
				if (!isAlreadyTalk(qs.getQuestVar2(), 6) && (spawn_Bert == null || !spawn_Bert.isSpawned())) {
					spawn_Bert = QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 205214,
							npc.getX(), npc.getY(), npc.getZ(), npc.getHeading()); // Bert's Ghost
				}
				return true;
			}
	...
	private void talkToGhost(QuestState qs, QuestEnv env, Npc npc, Player player, int pnjId) {
		...
		// if (qs.getQuestVar2() == 0 || qs.getQuestVar2() == 1 || qs.getQuestVar2() ==
		// 2 || qs.getQuestVar2() == 4 ||
		// qs.getQuestVar2() == 8 || qs.getQuestVar2() == 16 || qs.getQuestVar2() == 32
		// || qs.getQuestVar2() == 64 ) {
		// ;
		// } else {
		qs.setQuestVarById(0, var + 1);
		...
		/*
		 * qs.setQuestVarById(0, var + 1); updateQuestStatus(env); removeQuestItem(env,
		 * 182209009, 1); PacketSendUtility.sendPacket(player, new
		 * SM_DIALOG_WINDOW(env.getVisibleObject().getObjectId(), 10));
		 * npc.getController().onDelete();
		 */
	}
}