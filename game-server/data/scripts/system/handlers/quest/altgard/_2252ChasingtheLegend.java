				...
				case USE_OBJECT:
					if (var == 0) {
						...
						PacketSendUtility.sendPacket(player,
								new SM_USE_OBJECT(player.getObjectId(), targetObjectId, 3000, 1));
						...
						ThreadPoolManager.getInstance().schedule(new Runnable() {
							...
							public void run() {
								...
								if (npc == null || npc.getObjectId() != targetObjectId)
									return;
								QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 210634,
										npc.getX(), npc.getY(), npc.getZ(), npc.getHeading()); // Minusha's Spirit
