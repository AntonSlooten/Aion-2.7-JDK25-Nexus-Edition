...
public class TvtEventAI2 extends NpcAI2 {
			...
			public void acceptRequest(Creature requester, Player responder) {
				...
				if (!tvt.getHolders().getPlayer(player)) {
					...
					PacketSendUtility.sendMessage(player,
							"[Ascension]Congratulations!You Registered, for More Info Click to NPC - Aernia");
	 ...
	 * @param player
	 *
	 *               void inRestRoom(final Player player) { String message =
	 *               "[Ascension]Tvt: You Registered in Tvt Event!\nDo You want
	 *               Teleport in [Ascension]Rest Room?"; RequestResponseHandler
	 *               responseHandler = new RequestResponseHandler(player){
	 * 
	 *               public void acceptRequest(Creature requester, Player responder)
	 *               { TeleportService.goInCircle(player, 300200000, 1, 176, 21,
	 *               144, (byte)0); }
	 * 
	 *               public void denyRequest(Creature requester, Player responder){
	 *               return; } }; boolean requested =
	 *               player.getResponseRequester().putRequest(902247,
	 *               responseHandler);
	 *               if(requested){PacketSendUtility.sendPacket(player, new
	 *               SM_QUESTION_WINDOW(902247, 0, message));return;}
	 * 
	 *               }
	 */
}
