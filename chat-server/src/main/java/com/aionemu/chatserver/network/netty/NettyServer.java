...
import com.aionemu.chatserver.utils.concurrent.NamedThreadFactory;
...
public final class NettyServer {
	...
	private ExecutorService loginToClientBossExecutor;

	private ExecutorService loginToClientWorkerExecutor;

	private ExecutorService loginToGameBossExecutor;

	private ExecutorService loginToGameWorkerExecutor;
	...
	private void initialize() {
		loginToClientBossExecutor = newNettyExecutor("chat-client-boss", 1);
		loginToClientWorkerExecutor = newNettyExecutor("chat-client-worker", workerCount());
		loginToGameBossExecutor = newNettyExecutor("chat-game-boss", 1);
		loginToGameWorkerExecutor = newNettyExecutor("chat-game-worker", workerCount());

		loginToClientChannelFactory = initChannelFactory(loginToClientBossExecutor, loginToClientWorkerExecutor);
		loginToGameChannelFactory = initChannelFactory(loginToGameBossExecutor, loginToGameWorkerExecutor);

		Channel loginToClientChannel = initChannel(loginToClientChannelFactory, Config.CHAT_ADDRESS,
	 ...
	 */
	private NioServerSocketChannelFactory initChannelFactory(ExecutorService bossExecutor,
	...
	private static ExecutorService newNettyExecutor(String threadName, int poolSize) {
	...
	public void shutdownAll() {
		...
		shutdownExecutor(loginToClientBossExecutor);
		shutdownExecutor(loginToClientWorkerExecutor);
		shutdownExecutor(loginToGameBossExecutor);
		shutdownExecutor(loginToGameWorkerExecutor);
	}
